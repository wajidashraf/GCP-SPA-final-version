// src/pages/EndorsementLetterPage.tsx
// Endorsement letter editor (GCPC channel) — reached at
// /requests/:id/endorsement-letter once the request is at status 8/11 (after
// HOC acceptance). Submit advances the request to status 12 (Endorsed / E).
//
// This is a letter, NOT a decision form: there is no conclusion code. The body
// comes from a matter-specific E template (src/components/letters); values we
// already hold auto-fill, the rest are manual variables the reviewer types in.
// Manual entries persist in gcp_endorselettertextcontent via a machine-readable
// marker so they can be re-edited.
//
// NOTE: deliberately a separate page from the Acknowledgement letter so the
// endorsement letter can carry its own structure/design — pending the template
// the user will provide. Until then it mirrors the standard letter editor.
...
const LETTER_TYPE = 'Endorsement';
const TARGET_STATUS = 12; // Endorsed (E)

export default function EndorsementLetterPage() {
  ...
  const template = useMemo(
    () => selectLetterTemplate('E', matterCode),
    [matterCode],
  );

  // Letter number format: companyCode/projectCode/soaLabel/title/E
  const letterNumber = [
    request?.companyCode,
    request?.projectCode,
    soaLabel,
  ...
  // ── Form state ────────────────────────────────────────────────────────────
  const existingText = request?.endorsementLetterText ?? '';
  ...
  useEffect(() => {
    if (!request) return;
    const raw = request.endorsementLetterText;
      ...
      <div className="container">
        ...
        >
          ...
          <Modal.Body>
            ...
            the request to <strong>Endorsed (E)</strong>. This action cannot be
            undone.
          </Modal.Body>
          <Modal.Footer>
            <button
