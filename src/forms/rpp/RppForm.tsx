...
import { submitRppRequest } from './api';
import type { RppFormState } from './types';

type RppFormProps = { matter: MatterChoice };

const DRAFT_KEY = 'rpp:new';
...
const RppForm = ({ matter }: RppFormProps) => {
  const { user } = useAuth();
  ...
  const { state, setState, clearDraft } = useFormDraft<RppFormState>(
    DRAFT_KEY,
    initial
  );
  ...
  const set = <K extends keyof RppFormState>(key: K, value: RppFormState[K]) =>
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const result = await submitRppRequest(state, {
