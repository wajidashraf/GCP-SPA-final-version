...
import { submitOtherRequest } from './api';
import type { OthersFormState } from './types';

type OthersFormProps = { matter: MatterChoice };
...
const OthersForm = ({ matter }: OthersFormProps) => {
  const { user } = useAuth();

  // Two matters (gcpc=9, gcp=13) share this form/table — key the draft per
  // channel so switching channels doesn't reuse the other's in-progress draft.
  const DRAFT_KEY = `others:${matter.channel}:new`;
  ...
  const { state, setState, clearDraft } = useFormDraft<OthersFormState>(
    DRAFT_KEY,
    initial
  );
  ...
  const set = <K extends keyof OthersFormState>(
    key: K,
    value: OthersFormState[K]
    ...
    },
    {
      ...
      render: () => (
        <Row className="g-3">
          ...
          <Col xs={12}>
            <TextAreaField
              name="descriptionOfMatters"
              label="Description of Matter"
              value={state.descriptionOfMatters}
              onChange={(e) => set('descriptionOfMatters', e.target.value)}
              rows={4}
              isRequired
            />
          </Col>
      ...
      validate: () => {
        ...
        if (!state.companyId) return 'Company is required.';
        if (!state.descriptionOfMatters.trim())
          return 'Description of Matter is required.';
        return null;
      },
    },
    {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const result = await submitOtherRequest(state, {
