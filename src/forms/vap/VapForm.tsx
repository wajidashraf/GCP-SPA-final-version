...
import { submitVapRequest } from './api';
import type { VapFormState } from './types';

type VapFormProps = { matter: MatterChoice };

const DRAFT_KEY = 'vap:new';
...
const VapForm = ({ matter }: VapFormProps) => {
  const { user } = useAuth();
  ...
  const { state, setState, clearDraft } = useFormDraft<VapFormState>(
    DRAFT_KEY,
    initial
  );
  ...
  const set = <K extends keyof VapFormState>(key: K, value: VapFormState[K]) =>
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const result = await submitVapRequest(state, {
